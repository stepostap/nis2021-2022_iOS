...
export const NotesScreen = () => {
    ...
    const storeData = async (value) => {
        try {
            ...
            await AsyncStorage.setItem('@todoID', JSON.stringify(currentID))
        } catch (e) {
            console.log(e)
        }
    }
    ...
    const createNote = (header, value) => {
        setID(prev => prev+1)
        setNotes(prev => [...prev, { id: currentID, title: header, noteText: value }])
    ...
    const getData = async () => {
        try {
            ...
            const valueID = await AsyncStorage.getItem('@todoID')
            if (valueID !== null) {
                setID(Number(valueID))
            }
        } catch (e) {
            console.log(e)
        }
    ...
    return (
        <View style={{ flex: 1, width: '100%', padding: 20 }}>
            {screen == 'notesScreen' && <>
                ...
                <View style={{ flex: 8, marginBottom: 20 }}>
                    <ScrollView>{notes.map((e) =>
                        <TouchableOpacity style={{ flexDirection: 'row', marginBottom: 15 }}
                         onPress={() => { setScreen('viewNote'), setCurrentNote(e) }} key={e.id}>
                ...
                back={() => setScreen('notesScreen')} /> || screen == 'viewNote' && <ViewNote changeNote={changeNote}
